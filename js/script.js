...
background.src = "img/avplane.jpg";
...
function sendVal(form) {
	_arousal = form.arousal.value;
	_valence = form.valence.value;
	_hue = form.hue.value;
	_saturation = form.saturation.value;
	_value = form.valuee.value;
	_volume = form.volume.value;
	draw(_arousal, _valence, _hue, _saturation, _value, _volume);
}

function draw(_arousal, _valence, _hue, _saturation, _value, _volume){
	var fillcolor = new HSVColour(_hue, _saturation, _value);
	var _x = 198+((Math.floor(_valence)+100)*(674-198)*0.005);
	var _y = 547-((Math.floor(_arousal)+100)*(547-95)*0.005);
	ctx.clearRect(0, 0, canvas.width, canvas.height);
	ctx.drawImage(background,0,0); 
	ctx.beginPath();
	ctx.arc(_x,_y,_volume,0,2*Math.PI);
	ctx.fillStyle = fillcolor.getCSSHexadecimalRGB();
    ctx.fill();
}
...
//code.stephenmorley.org // Color Library
